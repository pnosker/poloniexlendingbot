...
def init(cfg, api1, log1, data, maxtolend, dry_run1, analysis, notify_conf1):
    ...
    global sleep_time, sleep_time_active, sleep_time_inactive, min_daily_rate, max_daily_rate, spread_lend, \
        gap_bottom_default, gap_top_default,  min_loan_size, end_date, coin_cfg, min_loan_sizes, dry_run, \
        transferable_currencies, keep_stuck_orders, hide_coins, scheduler, gap_mode_default, \
        vday_threshold, vdays, wday_threshold, wdays, xday_threshold, xdays, yday_threshold, wdays, zday_threshold, zdays
    ...
    vday_threshold = Decimal(Config.get("BOT", "vdaythreshold", None, 0.003, 5)) / 100
    vdays = str(Config.get("BOT", "vdays", None, 2, 60))
    wday_threshold = Decimal(Config.get("BOT", "wdaythreshold", None, 0.003, 5)) / 100
    wdays = str(Config.get("BOT", "wdays", None, 2, 60))
    xday_threshold = Decimal(Config.get("BOT", "xdaythreshold", None, 0.003, 5)) / 100
    xdays = str(Config.get("BOT", "xdays", None, 2, 60))
    yday_threshold = Decimal(Config.get("BOT", "ydaythreshold", None, 0.003, 5)) / 100
    ydays = str(Config.get("BOT", "ydays", None, 2, 60))
    zday_threshold = Decimal(Config.get("BOT", "zdaythreshold", None, 0.003, 5)) / 100
    zdays = str(Config.get("BOT", "zdays", None, 2, 60))
...
def create_lend_offer(currency, amt, rate):
    ...
    if float(amt) > get_min_loan_size(currency):
        ...
        amt = "%.8f" % Decimal(amt)   
        if float(rate) > zday_threshold:
            days = zdays
        if float(rate) > yday_threshold:
            days = ydays
        if float(rate) > xday_threshold:
            days = xdays
        if float(rate) > wday_threshold:
            days = wdays
        if float(rate) > vday_threshold:
            days = vdays
        if vday_threshold == 0:
            days = '2'
        if wday_threshold == 0:
